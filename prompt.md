...
3. **Configure the Android build workflow**:
   ...
   - Ask/guide the user to open the GitHub repository Actions tab in their mobile web browser to easily download and install the compiled APK artifact on their phone (do NOT download the APK directly into the working directory).
...
2. **Explicit tool specification in brief**: When launching a specialized subagent, the main LLM MUST explicitly and accurately specify in the brief the exact allowed inspection tools (`read`, `search`, `outline`, `find`, `list`, `recent`, `git status/diff/log`, `diagnostics`) for reading/searching the codebase, and explicitly forbid modification tools (`no patch/edit/fileops/cut/extract`). The brief MUST instruct the subagent to use `create_file` exclusively for writing its report to the assigned `report_to` path (`reports/<subagent>-<slug>.md`).
